...
int main(){
    ...
    uint32_t ans = 0;
    uint32_t set_num= 1000,aver_card = 100000;
                ...
                for (size_t reg_num = 256; reg_num <= 256; reg_num *= 2)
                {
                    ...
                    for (double epsilon = 0.1; epsilon <= 1; epsilon += 0.1)
                    {
                        ...
                        for (size_t times_id = 0; times_id < times_num; times_id++)
                        {
                            ...
                            std::cout << "vSkt, LLC, ";
                            AARE[2] += multiset_test<5, vSkt<5, AlgorithmLLC>>(set_num, card, (size_t)(6.4 * row_num), reg_num, noise, epsilon);
                            std::cout << "rSkt, FM, ";
                            AARE[3] += multiset_test<32, rSkt<32, AlgorithmFM>>(set_num, card, row_num / 2, reg_num, noise, epsilon);
                            std::cout << "rSkt, FMS, ";
                            AARE[4] += multiset_test<32, rSkt<32, AlgorithmFMS>>(set_num, card, row_num / 2, reg_num, noise, epsilon);
                            std::cout << "rSkt, LLC, ";
                            AARE[5] += multiset_test<5, rSkt<5, AlgorithmLLC>>(set_num, card, (size_t)(3.2 * row_num), reg_num, noise, epsilon);
                            std::cout << "CMSkt, FM, ";
                            AARE[6] += multiset_test<32, CMSkt<32, AlgorithmFM>>(set_num, card, row_num / 2, reg_num, noise, epsilon);
                            std::cout << "CMSkt, FMS, ";
                            AARE[7] += multiset_test<32, CMSkt<32, AlgorithmFMS>>(set_num, card, row_num / 2, reg_num, noise, epsilon);
                            std::cout << "CMSkt, LLC, ";
                            AARE[8] += multiset_test<5, CMSkt<5, AlgorithmLLC>>(set_num, card, (size_t)(3.2 * row_num), reg_num, noise, epsilon);
                            std::cout << "mySkt, FM, ";
                            AARE[9] += multiset_test<32, mySkt<32, AlgorithmFM>>(set_num, card, row_num / 2, reg_num, noise, epsilon);
                            std::cout << "mySkt, FMS, ";
                            AARE[10] += multiset_test<32, mySkt<32, AlgorithmFMS>>(set_num, card, row_num / 2, reg_num, noise, epsilon);
                            std::cout << "mySkt, LLC, ";
                            AARE[11] += multiset_test<5, mySkt<5, AlgorithmLLC>>(set_num, card, (size_t)(3.2 * row_num), reg_num, noise, epsilon);
                            delete[] card;
                        }
                        std::cout << "vSkt, FM, AARE:" << AARE[0] / times_num << std::endl;
                        std::cout << "vSkt, FMS, AARE:" << AARE[1] / times_num << std::endl;
