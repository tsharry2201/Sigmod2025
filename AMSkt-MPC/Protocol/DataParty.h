...
public:
  ...
  */
  ...
  int gene_discrete_gaussian(double sigma);
  double cal_var_epsilon_d(int d, double sigma, double sensitivity);
  double cal_sigma(int d, double epsilon_d, double sensitivity);
  double converse_epsilon_d(double epsilon, double delta);
  int get_BucketNum_and_Rank(__uint128_t out, int * bucket_index, int w, int bits);
