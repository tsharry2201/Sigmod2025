...
void CP::init_offline_data(){
    ...
    for(int i=0; i<nthreads; i++){
        ...
        ss << PREP_DATA_PREFIX << "/RandExps-p-30-P" << mynum;
        if (i){
            ss << "-" << i;
        }
...
void CP::bucket_bExtractZ(const std::vector<std::vector<Share<T>>>& src_fm, Share<T>& Z, const int m_start, int m_internal, int thread_num, bool multi_thread){
    ...
    Z += src_fm[src_fm.size()-1][0];//modified
    thread_num = 1;

    if(multi_thread && thread_num){
        pthread_mutex_lock(&mutex_go); 
        Go++;
        pthread_mutex_unlock(&mutex_go);
    }
...
void CP::benchmark(){
    ...
    */
    ...
    batch_mac_check();
    fm_approximateCount(share_Z,oM, the_w);
    double unique_countC = fm_approximateCount(share_Z,oM, the_w);
    ...
    */
    ...
    batch_mac_check();
    fm_approximateCount(share_Z,oM, the_w);
    double unique_countb = fm_approximateCount(share_Z,oM, the_w);
...
void CP::mergeShare(){
    int ofs_M = share_FS.size() - 1;//modified
...
void CP::multi_thread_bucket_CExtractZ(std::vector<std::vector<Share<T>>>& src_fm, Share<T>& Z){
    int ofs_M = src_fm.size()-1;//modified
    if(!ofs_M){
        return;
    }
...
template <class T>
double CP::fm_approximateCount(const Share<T>& Z, int M, int w){
    ...
    R = w - R;
    //int it = 0;
    double dist = 1.0/2, x0 = 100.0, ave = 0, dave = 0, ave_dist = 0;
    for(int i = 0; i < w-1; ++i)
    {
        ave = ave + pow((1.0-pow(1.0/2, i+1)), x0);
        dave = dave + pow((1.0-pow(1.0/2, i+1)), x0) * log((1.0-pow(1.0/2, i+1)));
        ave_dist = ave_dist + pow((1.0-pow(1.0/2, i+1)), x0+dist);
    }
    while((ave-R)*(ave_dist-R) > 0)
    {
        x0 = x0 - (ave - R)/dave;
        ave = 0;
        dave = 0;
...
bool DP_connector::data_collection(int socket_num, std::vector<std::vector<Share<gfp>>>& share_FS){
    ...
    share_FS.resize(oM + 1);//modified
    for(int m=0; m<oM; m++){
        share_FS[m].resize(w);
    }
    share_FS[oM].resize(1);//modified
...
bool DP_connector::share_value(std::vector<std::vector<Share<T>>>& des_vec, int socket_num){
    ...
    int ofs_M = des_vec.size() - 1;//modified
    int ofs_w = des_vec[0].size();
    int elements_size = ofs_M*ofs_w + 1;//modified
    ...
    y_value.unpack(share_stream);//modified
    a_mac = a_Array[elements_size-1].get_mac();//modified
    a_value = a_Array[elements_size-1].get_share();//modified
    tmp.assign_zero();//modified
    if(mynum == 0)//modified
    {
        a_value.add(y_value);//modified
        tmp.mul(y_value,alphai);//modified
        a_mac.add(tmp);//modified
    }
    else
    {
        tmp.mul(y_value,alphai);//modified
    ...
    des_vec[ofs_M][0].set_share(a_value);//modified
    des_vec[ofs_M][0].set_mac(a_mac);//modified
    return true;
}
